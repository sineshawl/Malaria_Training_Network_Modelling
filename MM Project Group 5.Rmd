---
title: "Network Analysis and SIS Epidemic Model"
author: "Group 5"
date: "`r Sys.Date()`"
output: 
  html_document:
        css: styles.css
        code_folding: hide   # or "hide"
        toc: true            # optional: table of contents
        toc_float:      # optional: floating TOC
          collapsed: true   # collapses sub-sections by default
          smooth_scroll: true
        toc_depth: 4
...
```{r library_loading, include=TRUE, message=FALSE, echo=FALSE}
suppressWarnings(library(DT))
suppressWarnings(library(plotly))
suppressWarnings(library(tidyr))
suppressWarnings(library(igraph)) # For graphs
suppressWarnings(library(ggplot2)) # For plots
suppressWarnings(library(dplyr)) # For manipulation
suppressWarnings(library(RColorBrewer)) # For colours
...
```{r defining the data, include=FALSE, warning=FALSE, message=FALSE}
...
```{r display the data,  include=TRUE, warning=FALSE, message=TRUE}
datatable(
  data,
  options = list(
...
```{r create_a_network,  include=TRUE, warning=FALSE, message=TRUE}
...
- Participant Interaction Network

- Nodes = participants (colored by country).

- Edges = interactions between participants.

- Kenya (green nodes) is centrally located, acting as a hub linking participants from multiple nations.

- Network density shows frequent cross-country connections.

- Highly connected nodes (superspreaders) are key drivers of potential disease spread.

--- 

# Part 2: SIS Epidemic Modelling

```{r sis_model,  include=TRUE, warning=FALSE, message=TRUE}
...
```{r simuation,  include=TRUE, warning=FALSE, message=TRUE}
...
```{r analysis_and_result,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
cat("a) Number of infected individuals:\n
    Day 2:", infected_day2, "people\n
    Day 4:", infected_day4, "people\n
    Day 8:", infected_day8, "people\n\n
    
b) Incidence on Day 8:", incidence_day8, "new infections\n\n")
...
cat("c) With masks from Day 5:\n
   Infections on Day 5 with masks:", infected_day5_masks, "L\n
...
```{r visulaization,  include=TRUE, warning=FALSE, message=TRUE}
...
```{r exercise_1,  include=TRUE, warning=FALSE, message=TRUE}
...
```{r exercise_1.1,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
```{r exercise_1.1.1,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
cat("b) Degree Centrality Comparison:\n
   Kenya participants - Mean degree:", round(mean(kenya_degrees), 2), "\n
   Other countries - Mean degree:", round(mean(other_degrees), 2), "\n
   Kenya participants are", round(mean(kenya_degrees)/mean(other_degrees), 1), "times more connected\n\n")
```
...
- Calculate and compare betweenness centrality for Kenya vs other countries

**Answer**   

```{r exercise_1.2,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
- Identify least 3 connected individuals

**Answer**   

```{r exercise_1.3,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
```{r exercise_2,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
- Why does the infection plateau? What's the epidemiological phenomenon?

Why does the infection plateau? What's the epidemiological phenomenon?
ANSWER
- The infection plateaus because the epidemic reaches an equilibrium (Beta=0.071, I=40), where no more people can be infected.
Once the susceptible pool is exhausted, the outbreak stops, regardless of how high the transmission rate (β) is. The plateau simply reflects the population limit, not the strength of transmission. 

```{r exercise_2.1,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
- What is R₀? How would you use the knowledge of R₀ for disease control? What's another way tomcalculate R₀ as Billy showed in class? 

What is R₀? How would you use the knowledge of R₀ for disease control? What's another way to calculate R₀ as Billy showed in class? 
Answers
...
```{r exercise_3,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
cat("d) Early intervention is more effective because:\n
  - Smaller infected population to control\n
  - Prevents exponential growth phase\n
  - Delays can lead to overwhelming healthcare systems\n\n")
...
- Who are the top 6 most connected individuals abased on betweenness? Do they differ from those based on degree centrality? Comment on whether they are different or the same - why?

```{r exercise_3.1,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
```{r exercise_3.1_,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
- Calculate the superspreader effect based on betweenness centrality, and compare most connected as superspreader and random people as superspreader vs baseline? Do they differ from when using degree centrality?

**Solution:**

```{r exercise_3.2,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
## b) & c). Compare superspreader scenarios
# Scenario 1: Top 6 most connected as superspreaders
...
```{r exercise_3.2_,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
top3_indices <- which(rownames(adj_matrix) %in% names(top6_connected)[1:3])
...
- How many infections are prevented if we consider betweenness centrality fo isolation? 
**Solution:**

```{r exercise_3.3,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
```{r network_structure,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
...
-  Read the following 
Based on your analysis, answer the following research question:
QUESTION: You are a public health official planning intervention strategies
for a respiratory disease outbreak in a diverse international community.
Given limited resources, you must choose ONE of the following strategies:

Strategy A: Implement universal masking for everyone starting on Day 3
Strategy B: Identify and isolate the top 3 most connected individuals immediately
Strategy C: Implement random testing and isolation of 6 individuals on Day 1

```{r exercise_4,  include=TRUE, warning=FALSE, message=TRUE, results = "asis"}
